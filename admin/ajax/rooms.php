...
if(isset($_POST['edit_room'])){
    ...
    $del_features = delete_db("DELETE FROM `room_features` WHERE `room_id`=?", [$frm_data['room_id']],'i');
    $del_facilities = delete_db("DELETE FROM `room_facilities` WHERE `room_id`=?", [$frm_data['room_id']],'i');
...
if (isset ($_POST['get_room_images'])) {
    ...
    while($row = mysqli_fetch_assoc($res)){
        if($row['thumb']==1){
            $thumb_btn = "<i class='bi bi-check-lg text-light bg-success px-2 pi-1 rounded fs-5'></i>";
        }else{
            $thumb_btn = "
...
if (isset ($_POST['remove_room'])) {
    ...
    $res2 = delete_db("DELETE FROM `room_images` WHERE `room_id`=?",[$frm_data['room_id']],'i');
    $res3 = delete_db("DELETE FROM `room_features` WHERE `room_id`=?",[$frm_data['room_id']],'i');
    $res4 = delete_db("DELETE FROM `room_facilities` WHERE `room_id`=?",[$frm_data['room_id']],'i');
